...
Perception agent — sensor fusion, confidence-weighted observations, health monitoring, built on cuda-equipment
...
- `SensorAgent` — core data structure
- `FusedObservation` — core data structure
...
```bash
# Clone
git clone https://github.com/Lucineer/cuda-sensor-agent.git
cd cuda-sensor-agent
...
- `Observation` — see source for methods
- `SensorAgent` — see source for methods
- `Agent for SensorAgent` — see source for methods
